...
rtosStatus_t osSemaphoreNew (uint32_t max, uint32_t init, 
						const rtosSemaphoreAttr_t* attrs, rtosSemaphoreHandle_t* semaphore) {
...
rtosStatus_t     osSemaphoreAcquire(rtosSemaphore_t* id, uint32_t timeout){
	...
	// desired time has been reached	
		...
		do {
			__enable_irq();
			__disable_irq();
		}  while ((uint32_t)(startTicks - rtosGetSysTickCount()) 
									< timeout && id->count <= 0);
